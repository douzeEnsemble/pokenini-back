...
- [ ] [haute] Secrets OAuth réels committés dans `.env.prod`
  Fichier : `.env.prod:43-46`
  Suggestion : Les variables `OAUTH_DISCORD_CLIENT_SECRET` et `OAUTH_GOOGLE_CLIENT_SECRET` contiennent des valeurs de production réelles. Ce fichier est commité dans le dépôt. Utiliser des secrets GitHub Actions injectés au build ou un vault. Remplacer les valeurs par des placeholders `!ChangeMe!` dans le fichier commité.
