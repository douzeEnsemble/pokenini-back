# Design: automated Composer updates workflow (pokenini-back)
...
replicated here because `pokenini-back`'s `updates:` Makefile target, composite actions
...
- `schedule`: daily cron, `37 4 * * *` (04:37 UTC — offset from `pokenini-api`'s `17 4 * * *`).
...
4. **Open/update PR** — `peter-evans/create-pull-request@v7`:
   ...
   - `labels: dependencies` (confirmed this label exists in `pokenini-back`)
...
- `PAT_TOKEN`: a fine-grained Personal Access Token scoped to `pokenini-back` with
