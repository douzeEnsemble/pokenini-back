# Composer Updates Automation Implementation Plan (pokenini-back)
...
- Cron: `37 4 * * *` (04:37 UTC daily — offset from `pokenini-api`'s `17 4 * * *` to avoid runner contention).
...
- Consumes: `secrets.PAT_TOKEN` (repository secret, must be created by the user out-of-band for **this** repo specifically — a fine-grained PAT scoped to `pokenini-back` with `contents: write` + `pull-requests: write`; GitHub secrets are repo-scoped, so this is separate from `pokenini-api`'s secret of the same name).
...
1. Create a fine-grained GitHub PAT scoped to `douzeEnsemble/pokenini-back` with `contents: write` and `pull-requests: write`, and add it as the repository secret `PAT_TOKEN`.
